...
class PreprocessResult(BaseModel):
    ...
    cleaned_text: str = Field(
        description="Normalized, cleaned RFP text to be used as the single source for requirements extraction"
    )
    removed_text: str = Field(
        default="",
        description="Administrative / out-of-scope text removed from the cleaned text (emails, addresses, etc.)",
    )
    key_requirements_summary: str = Field(
        default="",
        description="Short global summary of key requirements as markdown bullets (optional but helpful context)",
    )
    comparison_agreement: bool = Field(
        default=True,
        description="Whether the comparison step agreed that cleaned_text contains all necessary information",
    )
    comparison_notes: str = Field(
        default="",
        description="Notes from the comparison validation step about what was removed / kept",
    )
